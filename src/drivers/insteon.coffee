...
insteon.httpClient {host, port, user, password},  -> 
  log 'insteon connected to ' + host
  # insteon.on 'recvCommand', recvCommand

insteon.on 'error', (err) -> log 'insteon port err', err
...
module.exports =
  init: ->
    ...
    #     return
    #   {level, time} = val
    #   light = insteon.light insteonIdsByName[bulb]
    #   if cmd is 'dim'
